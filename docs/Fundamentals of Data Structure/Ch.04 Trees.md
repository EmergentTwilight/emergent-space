...
### FirstChild-NextSibiling Representation

- 每个 node 都有两个 pointer，第一个指向 firstchild，第二个指向 nextsibiling
- 表达**不唯一**，子节点的顺序是不重要的

![Pasted image 20240319185741.png](Pasted-image-20240319185741.png)
...
- 完全二叉树中，**叶子节点的个数比 2 度节点个数多 1** $n_0=n_2+1$
	- ![Pasted image 20240422190953.png](Pasted-image-20240422190953.png)
	- 或者这样理解
		- 如果没有二度节点，只会有一个叶子
...
### idea 1: abandoned 空间太大了，其实操作很简单，真正建立树的结构没有用
...
- There exists a binary tree with 2016 nodes in total, and with 16 nodes having only one child. **F**
	- 根据 [Properties of Binary Trees](#properties-of-binary-trees) 进行推导得出 $2n_2=1999$ 除不尽，所以不可能
